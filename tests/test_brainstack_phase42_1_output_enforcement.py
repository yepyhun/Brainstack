...
from brainstack.output_contract import validate_output_against_contract
...
def test_non_repairable_typed_invariants_report_blocked_status():
    result = validate_output_against_contract(
...
def test_provider_trace_records_blocked_final_output_validation(tmp_path):
    provider = _make_provider(tmp_path, "phase42-1-blocked")
    try:
        ...
        provider.record_output_validation_delivery(result, delivered_content="Kuldes blokkolva")
        ...
        assert final["delivered_status"] == "blocked"
        assert final["delivered_blocked"] is True
    finally:
        provider.shutdown()
