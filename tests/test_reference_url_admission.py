...
def test_transcript_trim_preserves_exact_url_literals() -> None:
    text = (
        "Remember this as a repo URL, do not open it, and preserve the exact "
        "reference even though the surrounding explanation intentionally exceeds "
        "the preview limit: https://example.com/org/resource-x"
    )
...
def test_turn_summary_preserves_user_url_when_assistant_ack_is_short() -> None:
    summary = build_turn_summary(
        "Remember this as a repo URL, do not open it: https://example.com/org/resource-x",
        "Recorded.",
        max_len=48,
    )
