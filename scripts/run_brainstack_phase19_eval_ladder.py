#!/usr/bin/env python3
"""Run the bounded Phase 19 L3 eval ladder without requiring a live rebuild."""
...
def _build_overlay() -> tempfile.TemporaryDirectory[str]:
    tmp_dir = tempfile.TemporaryDirectory(prefix="brainstack-phase19-eval-")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the bounded Brainstack Phase 19 eval ladder.")
    parser.add_argument(
        "--gate",
        choices=["a", "b", "c", "all"],
        default="all",
    ...
    try:
        if args.gate in {"a", "all"}:
            rc = _run(
                "Gate A: Chroma backend and semantic seam",
                [
                    str(python_bin),
                    "-m",
                    "pytest",
        ...
        if args.gate in {"b", "all"}:
            rc = _run(
                "Gate B: corpus packing and retrieval contract",
                [
                    str(python_bin),
                    "-m",
                    "pytest",
    ...
    if failures:
        print("\nPhase 19 eval ladder failed:", ", ".join(failures))
        return 1

    print("\nPhase 19 eval ladder passed for requested gates.")
    return 0
