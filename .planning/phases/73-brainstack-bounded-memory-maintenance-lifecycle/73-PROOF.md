...
python3 -m py_compile brainstack/maintenance.py brainstack/__init__.py tests/test_memory_maintenance.py tests/test_agent_tool_surface.py
```
...
```text
passed_count 26
PASS tests.test_memory_maintenance.test_consolidate_apply_rebuilds_only_derived_semantic_index
PASS tests.test_memory_maintenance.test_consolidate_dry_run_reports_candidates_without_mutating_truth
PASS tests.test_memory_maintenance.test_consolidate_rejects_unsupported_apply_class_without_mutation
...
PASS for Phase 73 scope.

Maintenance is useful, bounded, receipt-backed, dry-run-first, and apply mode is limited to derived semantic index rebuild with no durable-truth deletion.
